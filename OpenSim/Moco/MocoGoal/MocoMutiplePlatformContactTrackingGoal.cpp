/* -------------------------------------------------------------------------- *
 * OpenSim Moco: MocoMultiplePlatformContactTrackingGoal.cpp                                  *
 * -------------------------------------------------------------------------- *
...
#include "MocoMutiplePlatformContactTrackingGoal.h"
#include <OpenSim/Simulation/Model/SmoothSphereClosedHalfSpaceForce.h>
#include <OpenSim/Simulation/Model/SmoothSphereHalfSpaceForce.h>

using namespace OpenSim;
using RefPtrMSF = SimTK::ReferencePtr<const MocoScaleFactor>;

MocoMultiplePlatformContactTrackingGoalGroup::MocoMultiplePlatformContactTrackingGoalGroup() {
    constructProperties();
}
...
MocoMultiplePlatformContactTrackingGoalGroup::MocoMultiplePlatformContactTrackingGoalGroup(
        ...
        MocoMultiplePlatformContactTrackingGoalGroup(contactForcePaths, externalForceName) {
    for (const auto& path : altFramePaths) {
        append_alternative_frame_paths(path);
    }
}

void MocoMultiplePlatformContactTrackingGoalGroup::constructProperties() {
...
void MocoMultiplePlatformContactTrackingGoal::constructProperties() {
...
void MocoMultiplePlatformContactTrackingGoal::setExternalLoadsFile(
        const std::string& extLoadsFile) {
    updProperty_external_loads().clear();
    set_external_loads_file(extLoadsFile);
}

void MocoMultiplePlatformContactTrackingGoal::setExternalLoads(const ExternalLoads& extLoads) {
    updProperty_external_loads_file().clear();
    set_external_loads(extLoads);
}

void MocoMultiplePlatformContactTrackingGoal::addScaleFactor(const std::string& name,
...
void MocoMultiplePlatformContactTrackingGoal::initializeOnModelImpl(const Model& model) const {
    ...
    for (int ig = 0; ig < getProperty_contact_groups().size(); ++ig) {
        ...
        for (int ic = 0; ic < group.getProperty_contact_force_paths().size();
            ...
            // Try casting to SmoothSphereHalfSpaceForce first.
            // If that fails, cast to SmoothSphereClosedHalfSpaceForce.
            // No other contact forces are allowed.
            auto contactForceGeneric =
                model.getComponent<Force>(path);
            const auto contactForce = dynamic_cast<SmoothSphereHalfSpaceForce*>(&contactForceGeneric);
            if (contactForce == NULL) {
                const auto contactForce = dynamic_cast<SmoothSphereClosedHalfSpaceForce*>(&contactForceGeneric);
                int recordOffset = findRecordOffset(
                        group, *contactForce, extForce.get_applied_to_body());
                groupInfo.closed_contacts.push_back(
...
int MocoMultiplePlatformContactTrackingGoal::findRecordOffset(
...
int MocoMultiplePlatformContactTrackingGoal::findRecordOffset(
    ...
    // Is the ExternalForce applied to the sphere's body?
    const auto& sphereBase = contactForce.getConnectee<ContactSphere>("sphere")
                                     .getConnectee<PhysicalFrame>("frame")
                                     .findBaseFrame();
    const std::string& sphereBaseName = sphereBase.getName();
    if (sphereBaseName == appliedToBody) {
...
void MocoMultiplePlatformContactTrackingGoal::calcIntegrandImpl(
    ...
    for (int ig = 0; ig < (int)m_groups.size(); ++ig) {
        ...
        // Model force.
        // First model opens half space forces
        ...
        // Add the closed half space forces
        for (const auto& entry : group.closed_contacts) {
            Array<double> recordValues = entry.first->getRecordValues(state);
            const auto& recordOffset = entry.second;
            for (int im = 0; im < force_model.size(); ++im) {
                force_model[im] += recordValues[recordOffset + im];
...
void MocoMultiplePlatformContactTrackingGoal::printDescriptionImpl() const {
